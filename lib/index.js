...
const createFile = ({
    ...
    const isExplicitlyListedForCompilation = F.compose(
        x => !F.isEmpty(x),
        F.filter(p => filepath.indexOf(p) >= 0)
    )(explicitCompilePaths);
    const shouldSkip = !isExplicitlyListedForCompilation && skipCompileForFileExtensions.indexOf(fileExt) >= 0;
