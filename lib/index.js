...
const createFile = (parentOutputDir, data, skipCompileForFileExtensions = []) => ({ templateFilepath, filename, shortpath, directory }) => {
    ...
    return mkdirpSafe(dirpath)
        .then(() => {
            ...
            const promise = shouldSkip
                ? read(templateFilepath)
                : compile(templateFilepath, data);
            return promise.then(content => ({ content, shortpath }));
        })
